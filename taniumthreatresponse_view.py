...
def get_events(headers, data):
    ...
    for event in data:
        ...
        for head in headers:
            ...
            event_details.append({"data": data, "contains": contains_map.get(head, None) if data else None})
...
def display_events(provides, all_app_runs, context):
    ...
    headers_map = {
        "combined": ["id", "pid", "process_table_id", "type", "operation", "timestamp", "detail"],
        "dns": [
            "id",
            "pid",
            "process_table_id",
        ...
        "file": ["id", "pid", "process_table_id", "file", "operation", "user_name", "group_name", "details", "timestamp"],
        "network": [
            "id",
            "pid",
            "process_table_id",
...
def get_process(headers, data):
    contains_map = {"id": ["threatresponse process table id"], "process_path": ["file path"]}
...
def display_process_tree(provides, all_app_runs, context):
    ...
    for summary, action_results in all_app_runs:
        for result in action_results:
            ...
            results.append({"data": final_result, "parameter": result.get_param(), "message": result.get_message()})
